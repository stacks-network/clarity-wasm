...
use crate::words::{ComplexWord, SimpleWord};
...
pub trait ChargeGenerator {
    ...
    fn cost_context(&self) -> Option<&ChargeContext>;
    ...
    ///
    /// Code will be generated under the following conditions:
    ///
    /// - The cost of the word is known
    /// - [`CostTrackingContext::should_emit`] is `true`
    fn charge(&self, instrs: &mut InstrSeqBuilder, word_name: ClarityName, n: impl Into<Scalar>) {
        if let Some(ctx) = self.cost_context() {
            if let Some(cost) = clar3::WORD_COSTS.get(&word_name) {
                ctx.emit(instrs, cost, n);
            }
        }
    }
}
...
// NOTE: We don't guarantee that the `LocalId` is of `ValType::I32`. We do this
//       to avoid having to introduce `Module` in `ChargeContext`, and avoid
//       having to handle an error condition originating from programmer mistake.
//       This means the implementer should be extra careful to ensure the local
//       is the correct type.
...
impl ChargeContext {
    fn emit(&self, instrs: &mut InstrSeqBuilder, cost: &WordCost, n: impl Into<Scalar>) {
        let n = n.into();
    ...
    fn emit_with_caf(
        ...
        match params {
            Caf::Constant(n) => caf_const(instrs, global, self.runtime_error, err_code, n),
            Caf::Linear { a, b } => {
                caf_linear(instrs, global, self.runtime_error, err_code, n, a, b)
            }
            Caf::NLogN { a, b } => caf_nlogn(instrs, global, self.runtime_error, err_code, n, a, b),
            Caf::None => {}
        }
    }
}
...
mod test_caf {
    ...
    fn module_with_caf<S: Into<Scalar>>(caf: impl FnOnce(LocalId) -> (Caf, S)) -> Vec<u8> {
        ...
        match caf {
            Caf::Constant(n) => caf_const(&mut body, cost_global, error, ERR_CODE, n),
            Caf::Linear { a, b } => {
                caf_linear(&mut body, cost_global, error, ERR_CODE, scalar, a, b)
            }
            Caf::NLogN { a, b } => caf_nlogn(&mut body, cost_global, error, ERR_CODE, scalar, a, b),
            Caf::None => {}
        }
        body.local_get(arg);
