    ...
    fn charge(
        ...
        if let Some((ctx, module)) = self.cost_context() {
            match n {
                Scalar::Compile(_) => {}
                Scalar::Run(id) => {
                    let local = module.locals.get(id);
                    match local.ty() {
                        walrus::ValType::I32 => {}
                        ty => {
            ...
            match maybe_word_cost {
                Some(cost) => ctx.emit(instrs, cost, n),
                None => {
                    return Err(GeneratorError::InternalError(format!(
...
impl ChargeContext {
    fn emit(&self, instrs: &mut InstrSeqBuilder, cost: &WordCost, n: Scalar) {
        self.emit_with_caf(
            instrs,
            cost.runtime,
            self.runtime,
    ...
    fn emit_with_caf(
        ...
        match params {
            ...
            Caf::LogN { a, b } => caf_logn(instrs, global, self.runtime_error, err_code, n, a, b),
...
mod test_caf {
    ...
    fn module_with_caf<S: Into<Scalar>>(caf: impl FnOnce(LocalId) -> (Caf, S)) -> Vec<u8> {
        ...
        match caf {
            ...
            Caf::LogN { a, b } => caf_logn(&mut body, cost_global, error, ERR_CODE, scalar, a, b),
