...
- `principal`: `i32` offset in call stack, `i32` length; call stack contains 21 bytes for standard principal (1 byte version, 20 byte Hash160) followed by a 1 byte indicating the length of the contract name, which, if non-zero, is followed by the contract name string.
