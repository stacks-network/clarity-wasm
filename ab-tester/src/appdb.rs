...
impl AppDb {
    ...
    pub fn db_mut(&mut self) -> &mut SqliteConnection {
        &mut self.conn
    }

    pub fn get_contract_id(&mut self, contract_identifier: &QualifiedContractIdentifier) -> Result<Option<i32>> {
    ...
    pub fn get_var_id(&mut self, contract_id: i32, key: &str) -> Result<Option<i32>> {
    ...
    pub fn insert_var_instance(&mut self, contract_execution_id: i32, contract_var_id: i32, value: &[u8]) -> Result<ContractVarInstance> {
        let result = insert_into(contract_var_instance::table)
            ...
            .get_result::<ContractVarInstance>(&mut self.conn)
            .unwrap();

        Ok(result)
    }

    pub fn insert_execution(&mut self, block_id: i32, transaction_id: &[u8], contract_id: i32) -> Result<ContractExecution> {
        let result = insert_into(contract_execution::table)
            ...
            .get_result::<ContractExecution>(&mut self.conn)
            .unwrap();

        Ok(result)
    }

    pub fn get_var_latest(&mut self, contract_id: i32, key: &str) -> Result<Option<Vec<u8>>> {
    ...
    pub fn insert_block(&mut self, environment_id: i32, id: i32, height: i32, hash: &[u8], marf_trie_root_hash: &[u8]) -> Result<Block> {
        let result = insert_into(block::table)
            ...
            .get_result::<Block>(&mut self.conn)
            .unwrap();

        Ok(result)
    }

    pub fn insert_environment(&mut self, runtime_id: i32, name: &str) -> Result<Environment> {
        let result = insert_into(environment::table)
            .values((
                environment::runtime_id.eq(runtime_id),
                environment::name.eq(name)
            ))
            .get_result::<Environment>(&mut self.conn)
            .unwrap();

        Ok(result)
    }

    pub fn insert_contract(&mut self, block_id: i32, contract_id: &str, source: &str) -> Result<Contract> {
        ...
        let result = insert_into(contract::table)
            ...
            .set((
                contract::block_id.eq(block_id),
                contract::source.eq(&compressed_source)))
            .get_result::<Contract>(&mut self.conn)
            .unwrap();
