use ratatui::{widgets::{TableState, Row, Table, Borders, BorderType}, text::{Span, Line}, prelude::{Constraint, Alignment, Rect}};
use color_eyre::eyre::Result;

use crate::{cli::console::{theme::Theme, data::Block, components::{Component, title_span}, tui::Frame}, context::BlockCursor};
...
impl<'a> Component for BlocksScreen<'a> {
    fn draw(&mut self, f: &mut Frame<'_>, area: Rect, theme: &Theme) -> Result<()> {
        let header_cells = ["height", "hash", "timestamp", "tx count"]
            .iter()
            .map(|h|
                Line::from(Span::styled(h.to_owned(), theme.blocks_table_header))
            );
        ...
        let rows = self.blocks.iter().enumerate().map(|(idx, block)| {
            let style1 = Theme::color_table_cell(theme.blocks_table_row_top_1, theme.blocks_table_row_bottom, idx as u8, area.height.wrapping_sub(1));
            let style2 = Theme::color_table_cell(theme.blocks_table_row_top_2, theme.blocks_table_row_bottom, idx as u8, area.height.wrapping_sub(1));
        ...
        let table = Table::new(rows)
            ...
            .block(ratatui::widgets::Block::default()
                ...
                .title(title_span("Blocks", theme.blocks_title, theme.blocks_border))
            )
